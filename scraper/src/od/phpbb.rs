...
impl PHPBB{
    ...
    fn selector_present(res:&str) -> bool{
        Document::from(res).find(Name("body").and(Attr("id","phpbb"))).any(|node| node.eq(&node))
    }
    /// Check meta description tag for id
    fn description_id(res:&str) -> bool{
        Document::from(res).find(Name("meta").and(Attr("name","description"))).any(|node| {
            match node.attr("content"){
    ...
    fn header_title(res:&str)->bool{
        Document::from(res).find(Name("div").descendant(Name("h1"))).any(|node| node.text().contains(IDENTIFIER))
    }
