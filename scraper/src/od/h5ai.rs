...
use select::predicate::{Predicate,Name,Class,Attr};
...
impl H5AI{
    ...
    fn tree_id(res:&str) ->bool{
        Document::from(res).find(Name("div").and(Attr("id","tree"))).any(|node| node.eq(&node))
    }
    /// Search the meta tag for an ID
    fn meta_id(res:&str)->bool{
        Document::from(res).find(Name("meta")
    ...
    fn js_id(res:&str)->bool{
        Document::from(res).find(Name("script"))
