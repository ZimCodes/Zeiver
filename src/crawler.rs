...
impl WebCrawler {
    ...
    pub fn scraper_task(&self,client:&reqwest::Client,path:Option<PathBuf>)-> Box<scrape::Scraper>{
        let path = match path{
    ...
    pub fn downloader_task(&self,client:&reqwest::Client,scraper:Box<scrape::Scraper>){
        let save = self.opts.output.to_str().expect("Cannot parse PathBuf into a &str in downloader task.");
        WebCrawler::run_downloader(&client, scraper, save, self.opts.cut_dirs, self.opts.tries,self.opts.wait,self.opts.retry_wait,self.opts.no_dirs,self.opts.random_wait)
    }
    pub fn recorder_task(&self,scraper:Box<scrape::Scraper>){
        let save = self.opts.output.to_str().expect("Cannot parse PathBuf into a &str in downloader task.");
        let recorder = record::Recorder::new(save,scraper,self.opts.verbose);
        recorder.run();
    }
    ...
    fn run_scraper(client:&reqwest::Client,path:&str,accept:&Option<String>,reject:&Option<String>,depth:usize,tries:u32,wait:Option<f32>,retry_wait:f32,is_random:bool,verbose:bool)
                   -> Box<scrape::Scraper>
    {
        let mut scraper = Box::new(scrape::Scraper::new());
    ...
    fn run_downloader(client:&reqwest::Client,scraper:Box<scrape::Scraper>,save:&str,cuts:u32,tries:u32,wait:Option<f32>,retry_wait:f32,use_dir:bool,is_random:bool){
        let downloader = download::Downloader::new(save,cuts,tries,wait,retry_wait,use_dir,is_random);
