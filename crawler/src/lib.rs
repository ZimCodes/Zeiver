...
impl WebCrawler {
    ...
    pub fn scraper_task(&self,client:&reqwest::Client,path:Option<PathBuf>)-> scraper::Scraper{
        let path = match path{
    ...
    pub fn downloader_task(&self,client:&reqwest::Client,scraper:Rc<scraper::Scraper>){
    ...
    pub fn recorder_task(&self,scraper:Rc<scraper::Scraper>,recorder_id:usize){
        let save = self.opts.output.to_str().expect("Cannot parse PathBuf into a &str in downloader task.");
        let mut recorder = recorder::Recorder::new(save, scraper, self.opts.verbose);
    ...
    fn run_downloader(client:&reqwest::Client,scraper:Rc<scraper::Scraper>,save:&str,cuts:u32,tries:u32,wait:Option<f32>,retry_wait:f32,
                      use_dir:bool,is_random:bool,verbose:bool){
        let downloader = downloader::Downloader::new(save,cuts,tries,wait,retry_wait,use_dir,is_random,verbose);
        downloader.start(client,scraper);
    }
    /// Retrieves urls from an input file
    pub fn get_links(path:Option<PathBuf>) ->Vec<PathBuf>{
        scraper::Scraper::links_from_file(path.unwrap().to_str().expect("Cannot parse links from file into a string"))
    }
}
