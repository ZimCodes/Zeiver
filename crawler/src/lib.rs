...
impl WebCrawler {
    ...
    async fn run_scraper(
        ...
        if let Err(e) = scraper
            .run(
                client, path, accept, reject, depth, tries, wait, retry_wait, is_random, verbose,
            )
            .await
        {
            panic!("{}", e.to_string());
    ...
    async fn run_downloader(
        ...
        let downloader = downloader::Downloader::new(
            save, cuts, tries, wait, retry_wait, use_dir, is_random, verbose,
        )
        .await;
