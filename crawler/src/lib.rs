...
impl WebCrawler {
    ...
    pub async fn print_header(&self, client: &reqwest::Client, url: PathBuf) -> Result<(), reqwest::Error> {
        let url = url.to_string_lossy();
        let header = self.opts.print_header.as_ref().unwrap();
        grabber::Http::print_header(header, client, url.as_ref(), self.opts.tries, self.opts.wait, self.opts.retry_wait, self.opts.random_wait, self.opts.verbose).await
    }
    pub async fn print_all_headers(&self, client: &reqwest::Client, url: PathBuf) -> Result<(), reqwest::Error> {
        let url = url.to_string_lossy();
        grabber::Http::print_headers(client, url.as_ref(), self.opts.tries, self.opts.wait, self.opts.retry_wait, self.opts.random_wait, self.opts.verbose).await
    }
}
